...
1. Guesses the symbol's location in that file
    - If it can't it defaults to the beginning of the file
1. Opens the file in a new tab
1. If any of the previous steps failed it does nothing

The guessing part is troublesome but in practice it works well enough. It depends on the package having a fairly standard structure.
...
## Usage
The project you're working on needs the following options:
...
```
    ghc-options:
        ...
        -hiedir=.hie
```
Currently the location is hardcoded to be ".hie" from the cabal project root.
The hie files need to be up-to-date enough, because symbol lookup happens through these files.

Then cd into the cli folder and install the cli tool with
`cabal install --overwrite-policy=always`

Verify that hls-lookup is in your path.
